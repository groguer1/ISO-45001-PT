...
(function (global) {
  ...
  var TITLES = [
    'Fundamentos de SST e Marco Legal Brasileiro',
    'A Norma ABNT NBR ISO 45001:2018',
    'Contexto da Organização e Liderança em SST',
    'Identificação de Perigos e Avaliação de Riscos',
    'Planejamento e Objetivos SST',
    'Suporte e Operação SST',
    'Investigação de Incidentes e CAT',
    'Auditoria Interna e Revisão pela Direção',
    'Processo de Certificação ISO 45001 no Brasil',
    'Gestão de Empreiteiros e Terceirizados no Brasil',
    'Casos Práticos por Setor no Brasil',
    'Projeto Final: Implante o SGSST'
  ];

  function file(n) { return 'iso45001-modulo' + n + '-pt.html'; }
  ...
  function buildDocumentPDF(title, contentText, filename) {
    ...
    function drawHeader() {
      ...
      doc.setFontSize(13);
      doc.text('ISOTECNIA · CONSULTOR ISO 45001 CERTIFICADO', marginX, 30);
    ...
    function drawFooter(pageNum) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text('Isotecnia — Curso Consultor ISO 45001 Brasil · Material de apoio', marginX, pageH - 28);
  ...
  var Deck = {
    ...
    lockedClick: function () {
      alert('Você precisa aprovar o quiz deste módulo (mínimo ' + PASS_THRESHOLD + '%) antes de continuar. Você pode revisar o conteúdo e refazer o quiz quantas vezes precisar.');
    },
    ...
    flashHint: function () {
      ...
      hint.textContent = '👉 Responda à pergunta para continuar.';
    ...
    tryAdvanceModule: function (opts) {
      ...
      if (!passed) {
        ...
        if (hint) {
          hint.style.display = 'block';
          hint.textContent = opts.lockMessage || ('🔒 Você precisa de pelo menos ' + PASS_THRESHOLD + '% para avançar. Revise o que falta e tente novamente.');
        }
        return;
      }
      if (this.moduleNum < TOTAL_MODULES) {
        ...
        if (done) { done.style.display = 'block'; done.style.color = 'var(--verde)'; done.textContent = '🎉 Curso concluído! Você já pode baixar seu certificado.'; }
      }
    },
    ...
    updateNavBar: function () {
      ...
      if (this.isLastSlide()) {
        nextBtn.textContent = this.moduleNum === TOTAL_MODULES ? '✓ Finalizar curso' : 'Próximo módulo →';
      } else {
    ...
    buildSlideContent: function (slide) {
      ...
      if (!fn) { var d = document.createElement('div'); d.textContent = 'Tipo de slide desconhecido: ' + slide.type; return d; }
    ...
    render_callout: function (slide) {
      ...
      card.innerHTML =
        ...
        '<div class="cc-hint">Toque para ver o detalhe →</div></div></div>' +
    ...
    render_penal: function (slide) {
      ...
      card.innerHTML =
        ...
        '<div class="flip-front"><h3>⚖️ ' + esc(slide.title) + '</h3><div class="hint">Toque no cartão para ver o detalhe</div></div>' +
    ...
    render_caso: function (slide) {
      ...
      card.innerHTML =
        ...
        '<button class="caso-reveal-btn">Ver o que aconteceu depois →</button>' +
    ...
    render_descargable: function (slide) {
      ...
      card.innerHTML =
        ...
        '<p>' + esc(slide.desc) + '</p><button class="btn-dl">⬇ Baixar</button></div>';
    ...
    render_quiz: function (slide) {
      ...
      wrap.innerHTML =
        '<div class="quiz-progress">Pergunta ' + slide.num + ' de ' + slide.total + '</div>' +
    ...
    render_resumen: function (slide) {
      ...
      wrap.className = 'slide-inner';
      wrap.innerHTML =
        '<h2 style="text-align:center;">Resultado do quiz</h2>' +
        '<div class="resumen-score" style="color:' + (passed ? 'var(--verde)' : 'var(--rojo)') + '">' + score.pct + '%</div>' +
        '<div class="resumen-label">' + score.correct + ' de ' + score.total + ' corretas — ' +
        (passed ? '✅ Quiz aprovado, você pode avançar para o próximo módulo.' : '🔄 Você precisa de ' + PASS_THRESHOLD + '% para avançar.') + '</div>' +
        '<div class="resumen-actions">' +
        (passed ? '' : '<button class="btn-nav secundario" id="btnRetry">↺ Refazer quiz</button>') +
        '</div>';
